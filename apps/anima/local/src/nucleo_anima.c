...
static const a_alias_t APP_ALIAS[] = {
    ...
    { "dosbox",             { "dos", "giochi", "gioco", "emulatore", "games", "game", NULL } },
...
static int tool_calc(const char *raw, char tok[A_MAX_TOKENS][A_TOK_LEN], int ntok, bool en, anima_result_t *r)
{
    ...
    else { char num[40]; a_fmt_num(cv, num, sizeof(num)); snprintf(r->reply, sizeof(r->reply), en ? "It's %s." : "Fa %s.", num); }
    return 1;
}
...
{
    if (anima_solve(raw, en, r)) return 1;
    return tool_calc(raw, tok, ntok, en, r);
}
...
static int try_cascade(const char *q, bool en, anima_result_t *r)
{
    ...
    *r = l0_query(q, en);
    ...
    if (nucleo_anima_l1_query(q, en, false, r))            A_CASCADE_HIT(kind ? topic : q);
    if (kind && nucleo_anima_l1_query(topic, en, false, r)) A_CASCADE_HIT(topic);
    ...
    if (kind && !names_proper && topic[0] && strncmp(topic, en ? "what" : "cos", en ? 4 : 3) != 0) {
        ...
        if (nucleo_anima_l1_query(canon, en, false, r)) A_CASCADE_HIT(topic);
    }
    #undef A_CASCADE_HIT
    return 0;
}
...
static int a_topic_strip(const char *q, char *out, size_t outsz)
{
    ...
    static const char *const lead_conv[] = {           // conversational -> L1-first
        ...
        "i heard about","i heard of","i read about", NULL };
    static const char *const lead_formal[] = {         // formal definition -> strip but L0-first
        ...
        "what is the","what is a","what is an","what is","what are","what s","whats","who is","who was","who are", NULL };
    ...
    static const char *const tailnoise[] = { " to me", " for me", " to a beginner", " please", " per favore",
        ...
        " like i m five", " simply", " meglio", " un po", " work", " do", " mean", NULL };
...
anima_result_t nucleo_anima_query(const char *input, const char *lang)
{
    ...
    if (s_online_only) {
        ...
        if (avail && nucleo_anima_online_teacher_pure(q, en, &r)) {
            mem_update(&r);
            snprintf(s_mem.last_topic, sizeof(s_mem.last_topic), "%s", q);
            s_session.dirty = true;
            goto done;
    ...
    // teacher_pure returns 0 without a key, so offline/no-key falls through to the entity bio (best
    ...
    if (askable && nucleo_anima_online_available() && nucleo_anima_online_is_about(q, en)) {
        ...
        if (nucleo_anima_online_chat(q, ring_last_input(), s_session.last.reply, en, &r)) {
    ...
    // bio so "capitale di X" / "quando è nato X" gives the fact, not a summary. Online-only (else 0).
    if (nucleo_anima_online_fact(q, en, &r)) {
        mem_update(&r);
        snprintf(s_mem.last_topic, sizeof(s_mem.last_topic), "%s", q);
        s_session.dirty = true;
    ...
    // Offline-first — answers from the learned cache if we've seen it before; otherwise, only with
    // Wi-Fi, fetches a structured summary and REMEMBERS it so it's answerable offline next time.
    // Runs before the band so a real fetched fact beats a fuzzy "did you mean". Never fabricates.
    {
        char entity[64], slug[64];
        if (nucleo_anima_online_entity(q, en, entity, sizeof(entity), slug, sizeof(slug)) &&
            nucleo_anima_online_answer(entity, slug, en, &r)) {
            mem_update(&r);
    ...
    if (a_is_followup_q(q)) {
        if (nucleo_anima_online_available() &&
            nucleo_anima_online_chat(q, ring_last_input(), s_session.last.reply, en, &r)) {
    ...
    // missed. STRICT Wikipedia lookup (free, no key) so junk stays an honest miss. Before the teacher.
    if (nucleo_anima_online_entity_bare(q, en, &r)) {
        mem_update(&r);
        snprintf(s_mem.last_topic, sizeof(s_mem.last_topic), "%s", q);
        s_session.dirty = true;
        goto done;
    ...
    // Last resort: the cloud teacher (open-ended questions, if the user configured an API key).
    // Disabled by default -> returns 0 and we fall through to an honest miss. See docs/anima-online.md §4.
    if (nucleo_anima_online_teacher(q, en, &r)) {
        mem_update(&r);
        snprintf(s_mem.last_topic, sizeof(s_mem.last_topic), "%s", q);
        s_session.dirty = true;
        goto done;
