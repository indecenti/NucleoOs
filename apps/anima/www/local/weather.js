...
// ---- live forecast (Open-Meteo: keyless and CORS-open, so the BROWSER calls it directly) ----------
// NEVER cached (volatility law §6). Shared by the ANIMA web app — whose weather used to exist only on
// the Cardputer, so with the device busy or away "meteo brescia" got a generic "I don't know" — and the
// PC simulator (tools/serve-shell.mjs). Mirrors the firmware weather_fetch(): geocode, then the daily
// row for the asked day (start_date=end_date, so any day in the ~15-day horizon works).
// opts: { fetch = globalThis.fetch, headers = {}, online = true, now = new Date() }

const foldPlace = (s) => String(s || '').toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '');

// Geocode with a multi-word fallback: Open-Meteo's index misses "reggio emilia" (stored as "Reggio
// nell'Emilia"), so on a miss search the FIRST token and pick the candidate whose region/admin/country
// matches the remaining words. Mirrors the firmware's full-then-first-token retry.
export async function geocodePlace(city, lang, opts = {}) {
  ...
  const first = await search(city, 1);
  if (first.length && typeof first[0].latitude === 'number') return first[0];
  const toks = String(city).split(/\s+/).filter(Boolean);
  if (toks.length < 2) return null;
  const cand = (await search(toks[0], 10)).filter((c) => typeof c.latitude === 'number');
  if (!cand.length) return null;
  const rest = toks.slice(1).map(foldPlace);
  let best = cand[0], bestScore = -1;
  for (const c of cand) {
    const hay = foldPlace(`${c.name || ''} ${c.admin1 || ''} ${c.admin2 || ''} ${c.admin3 || ''} ${c.country || ''}`);
    const score = rest.reduce((s, t) => s + (hay.includes(t) ? 1 : 0), 0);
    if (score > bestScore) { bestScore = score; best = c; }
  }
  return best;   // the region-matched candidate if any, else the most populous match of the first token
}

// { place, code, tmax, tmin, precipProb?, tcur? } for `city` on today+dayOffset, or null.
export async function fetchForecast(city, dayOffset, lang, opts = {}) {
  const f = opts.fetch || globalThis.fetch;
  const now = opts.now instanceof Date ? opts.now : new Date();
  try {
    const hit = await geocodePlace(city, lang, opts);
    ...
    const r = await f(url, { headers: opts.headers || {} });
    if (!r.ok) return null;
    const j = await r.json();
    const dy = j?.daily;
    if (!dy || !Array.isArray(dy.time) || !dy.time.length || typeof dy.weather_code?.[0] !== 'number') return null;
    return {
      place: hit.name || city,
      ...
      tcur: dayOffset === 0 ? j?.current?.temperature_2m : undefined,
    };
  } catch (e) {
    if (e && e.name === 'AbortError') throw e;       // a stopped turn stays stopped
    return null;
  }
}

// The /api/anima-shaped result of a weather answer (tier 'remote', domain 'meteo' like the device's).
export function weatherResult(q, reply, confidence) {
  return { query: q, tier: 'remote', action: 'answer', intent: 'weather', confidence, state: 'idle',
           reply, learned: false, verified: true, domain: 'meteo' };
}
...
export async function weatherAnswer(q, lang, opts = {}) {
  ...
  if (p.tooFar)
    return weatherResult(q, en ? `I only have a ~15-day forecast — ${p.dateLabel} is too far ahead.`
                               : `Ho previsioni solo fino a ~15 giorni: ${p.dateLabel} è troppo lontano.`, 60);
  if (opts.online === false)
    return weatherResult(q, en ? 'I need internet to check the weather.' : 'Mi serve internet per controllare il meteo.', 45);
  const fc = await fetchForecast(p.city, p.dayOffset, lang, opts);
  if (!fc || typeof fc.tmax !== 'number' || typeof fc.tmin !== 'number')
    return weatherResult(q, en ? `I couldn't get the weather for ${p.city}.` : `Non riesco a recuperare il meteo per ${p.city}.`, 40);
  return weatherResult(q, formatWeather(p, fc.place, fc, lang), 90);
}
