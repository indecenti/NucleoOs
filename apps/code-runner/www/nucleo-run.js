...
//   await os.hw.ir.send({protocol,address,command}) await os.hw.wifi.scan() await os.hw.gpio.write({pin,value})
// Paths are resolved against caps.cwd when relative.

import { callCapability, isCapability, resolveOffline } from './nucleo-hw.js';   // hardware capability manifest (single source)
...
function workerMain() {
  ...
  // os.hw.<ns>.<action>(args) -> rpc('hw.<ns>.<action>'). A nested Proxy means ANY namespaced call
  // is forwarded to the host, which validates the id against the capability manifest — so the worker
  // never has to hardcode the hardware surface (one manifest drives it).
  // The 'then'/'catch'/'finally'/Symbol guards stop a namespace proxy from looking thenable (so
  // `await os.hw.ir` can't hang) — only the leaf call os.hw.ir.send(...) returns a real Promise.
  function notReal(k) { return typeof k !== 'string' || k === 'then' || k === 'catch' || k === 'finally'; }
  os.hw = new Proxy({}, { get: function (_t, ns) {
    if (notReal(ns)) return undefined;
    // os.hw.command('spegni la tv') — deterministic OFFLINE natural language -> capability (no LLM).
    if (ns === 'command') return function (text) { return rpc('hw.command', [String(text == null ? '' : text)]); };
    return new Proxy({}, { get: function (_t2, action) {
      if (notReal(action)) return undefined;
      return function (a) { return rpc('hw.' + ns + '.' + action, [a]); };
    } });
  } });
