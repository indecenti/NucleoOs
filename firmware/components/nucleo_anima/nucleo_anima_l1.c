...
// PERSON-SHARD bare-surname floor. A bare "chi è <surname>" scores BELOW L1_RESCUE_ABS when the card
// carries only the full name ("Richard Nixon" @0.64). Lower the floor for the person.bin shard ONLY, and
// ONLY when the query's salient token appears EXACTLY in the card (l1_entity_exact_covered) — so a
...
{
    // LEAK-PROOF: s_idx e' un FILE* statico. Se un path libera la directory (s_cdir) e fa ripartire la
    // ensure_index/load SENZA chiudere s_idx (es. lo stand-down L1 quando il device e' online: avviene a
    // OGNI query offline), questa fopen sovrascriverebbe un FD ancora APERTO -> 1 file descriptor perso per
    ...
    HLOG(TAG, "load K=%u D=%u N=%u dir=%u free=%u largest=%u",
         (unsigned)s_K, (unsigned)s_D, (unsigned)s_N, (unsigned)cdir_sz,
         (unsigned)heap_caps_get_free_size(MALLOC_CAP_DEFAULT),
         (unsigned)heap_caps_get_largest_free_block(MALLOC_CAP_DEFAULT));
    // STREAMED CENTROIDS: the K*D int8 centroid slab is NO LONGER pulled into a contiguous RAM block.
    // That ~18 KB malloc was the SOLE heap-churn OOM source — on a no-PSRAM, no-compaction heap it had to
    ...
    if (!s_cdir) {
        ESP_LOGW(TAG, "load OOM cdir=FAIL free=%u largest=%u",
                 (unsigned)heap_caps_get_free_size(MALLOC_CAP_DEFAULT),
                 (unsigned)heap_caps_get_largest_free_block(MALLOC_CAP_DEFAULT));
...
bool nucleo_anima_l1_init(void)
{
    ...
    if (!map_encoder_from_flash()) {
        ...
        if (s_enc) { fclose(s_enc); s_enc = NULL; }   // leak-proof: never overwrite an already-open FD
        s_enc = fopen(ENC_PATH, "rb");
...
#else
    // AKB5 sharded routing is the device default. The per-shard unload/reload still touches the heap, but
    // load_index degrades gracefully on malloc-fail (no crash); largest_free_block is watched via /api/heap.
    s_akb5_on = nucleo_anima_l1_akb5_available();   // device: a valid SD manifest transparently takes over
#endif
    // When AKB5 is active the flat cluster index is pure dead weight — it would be unloaded one line below
    // anyway — so do NOT even attempt to load it. That boot-time malloc of the ~18 KB flat centroid slab,
    // failing under early fragmentation, was the SOLE source of the 'load OOM centroids=FAIL' warning; the
    // knowledge itself comes from the streamed AKB5 shards regardless. Load the flat index only AKB5-absent.
    bool flat_ok = s_akb5_on ? false : load_index();
...
{
    if (s_cdir) return true;                       // already resident (directory loaded; centroids stream from file)
...
// Free the in-RAM cluster index (the small K*8 directory + the reconstructable encoder hot-row cache)
// and close the index file. Safe to call repeatedly; the next query reloads. Centroids are streamed,
// so there is no ~18 KB slab to hand back any more — the launcher mainly recovers the hot-row cache.
...
// The offline programmatic tier is DEAD WEIGHT whenever a stronger brain is already answering, so by
...
static bool l1_is_stop_word(const char *w)
{
    static const char *const stops[] = {
        // Italiano interrogative e verbi comuni
        "come", "dove", "quando", "perche", "cosa", "quanti", "quante", "quanto", "quanta", "quale", "quali",
        "parlami", "spiegami", "definizione", "significato", "scritto", "autore", "morto", "nato", "nata", "morta",
        "deceduto", "scomparso", "nascita", "morte", "funziona", "funzionano", "serve", "servono", "questo", "questa",
        "questi", "queste", "quello", "quella", "quelli", "quelle", "dici", "puoi", "potresti", "sapresti", "spiegare",
        "capire", "voglio", "conosci", "presente", "sentito", "parlare", "sapere", "qualcosa", "dire", "dimmi", "parla",
        ...
        "oceano", "oceani", "isola", "isole", "capitale", "capitali", "stato", "stati", "regione", "regioni",
        "presidente", "presidenti", "governo", "governi", "re", "regina", "regine", "popolo", "popolazione", "abitante", "abitanti",
        "anno", "anni", "mese", "mesi", "giorno", "giorni", "ora", "ore", "tempo", "meteo", "temperatura", "gradi",
        "calcola", "calcolo", "traduzione", "traduci", "dizionario", "inglese", "italiano", "francese", "tedesco", "spagnolo",
        "persona", "persone", "uomo", "uomini", "donna", "donne", "ragazzo", "ragazzi", "libro", "libri", "film", "musica",
        "canzone", "canzoni", "gioco", "giochi", "animale", "animali", "pianta", "piante", "fiore", "fiori",
        "pianeta", "pianeti", "stella", "stelle", "sole", "luna", "terra", "mondo", "universo",
        // English interrogatives and common verbs
        "what", "when", "where", "who", "why", "how", "does", "wrote", "born", "died", "death", "birth", "capital",
        "author", "about", "tell", "more", "everything", "explain", "could", "would", "should", "understand", "heard",
        "familiar", "dummies", "simple", "terms", "beginner", "please", "work", "mean", "actually", "really",
        "precisely", "exactly", "meaning", "definition", "define", "describe", "about", "this", "that", "these",
        "those", "know", "says", "said", "with", "from", "their", "them", "then",
        "every", "always", "never", "about", "after", "before", "under", "above", "inside", "outside",
        // Common English nouns
        "history", "language", "languages", "word", "words", "letter", "letters", "number", "numbers",
        "country", "countries", "city", "cities", "river", "rivers", "mountain", "mountains", "lake", "lakes", "sea", "seas",
        "ocean", "oceans", "island", "islands", "state", "states", "region", "regions",
        "president", "presidents", "government", "governments", "king", "queen", "queens", "people", "population", "inhabitant", "inhabitants",
        "year", "years", "month", "months", "day", "days", "hour", "hours", "time", "weather", "temperature", "degrees",
        "calculate", "calculation", "translation", "translate", "dictionary", "english", "italian", "french", "german", "spanish",
        "person", "persons", "man", "men", "woman", "women", "boy", "boys", "book", "books", "movie", "movies", "music",
        "song", "songs", "game", "games", "animal", "animals", "plant", "plants", "flower", "flowers",
        "planet", "planets", "star", "stars", "sun", "moon", "earth", "world", "universe",
        NULL
    };
    for (int i = 0; stops[i]; i++) {
        if (strcmp(w, stops[i]) == 0) return true;
    }
    return false;
}

static bool l1_word_in_fuzzy(const char *hay, const char *w)
{
    int wl = (int)strlen(w);
    if (wl == 0) return false;
    
    if (l1_word_in(hay, w)) return true;

    // Fuzzy matching solo per parole >= 8 caratteri per evitare collisioni geografiche come Iceland/Ireland.
    if (wl < 8) return false;

    for (const char *p = hay; *p; ) {
        while (*p && !isalnum((unsigned char)*p) && (unsigned char)*p < 0x80) p++;
        if (!*p) break;
        char cand[40]; int cl = 0;
...
static bool l1_proper_noun_uncovered(const char *query, const char *clow)
{
    ...
    for (const char *p = query; *p; ) {
        ...
        if (len >= 2) {
            ...
            if (f >= 'A' && f <= 'Z') {
                caps++;
                if (word_idx > 0) has_internal_caps = true;
            }
            word_idx++;
        }
    }
    
    bool trust_caps = true;
    if (words >= 3 && caps * 2 > words) trust_caps = false;
    if (!has_internal_caps) trust_caps = false;

    bool first = true;                                  // pass 2: uncovered proper noun
    for (const char *p = query; *p; ) {
        while (*p && !isalnum((unsigned char)*p) && (unsigned char)*p < 0x80) p++;
        if (!*p) break;
        ...
        if (!first && k >= 4) {
            ...
            if (is_proper) {
                if (!l1_word_in_fuzzy(clow, lw)) return true;     // named entity the answer never mentions (even fuzzy)
            }
        }
        first = false;
    }
...
// STRICT entity coverage for the person-shard low-floor rescue (l1_lexically_corroborated is damlev-
// TOLERANT, so it green-lights "newtron"~"neutron" — a misspelling detector, not an entity verifier).
// Here EVERY salient query word (>=4 chars, not a lead-in) must appear EXACTLY (no fuzz) as a word in the
// reply. So a correctly spelled surname rescues ("nixon" in "Richard Nixon"), but a typo-neighbour
// ("newtron","washingtom") or a fabricated name ("floonk zorblax") does NOT -> abstain.
static bool l1_entity_exact_covered(const char *query, const char *reply)
{
    ...
    for (const char *p = q; *p; ) {
        ...
        if (st) continue;
        salient++;
        for (int r = 0; r < rn; r++) if (!strcmp(w, rw[r])) { covered++; break; }   // EXACT word match (no damlev)
    }
    return salient >= 1 && covered == salient;
}
...
int nucleo_anima_l1_query(const char *text, bool en, bool want_detail, anima_result_t *out)
{
    ...
    // RAM policy: the offline brain stands down for this query when a stronger one is answering
    ...
    // 1) nearest TOPC centroids, STREAMED from the index file (the K*D slab is never RAM-resident). Each
    //    centroid's norm is computed once in this pass; reading K*D bytes sequentially per query is cheap
    //    (the shard file is already open) and buys a heap with no big contiguous block to lose under churn.
    int keep = L1_TOPC;
#ifdef ANIMA_HOST
    ...
    g_anima_stage = 0xCC;                          // DIAG: centroid scan (STREAMED from the index file)
    // Stream the K centroids from the index file in chunks rather than indexing a resident K*D slab:
    ...
    for (uint32_t c0 = 0; c0 < s_K; c0 += CCHUNK) {
        ...
        size_t want = (size_t)nc * s_D;
        if (fread(cbuf, 1, want, s_idx) != want) return 0;
        for (uint32_t r = 0; r < nc; r++) {
            const int8_t *cv = cbuf + (size_t)r * s_D;
            int32_t dot = 0, css = 0;
            for (uint32_t k = 0; k < s_D; k++) { dot += (int32_t)qv[k]*cv[k]; css += (int32_t)cv[k]*cv[k]; }
            float cos = (float)dot / (qn * sqrtf((float)css) + 1e-6f);
            uint32_t c = c0 + r;
...
#endif
    if (best_ansoff < 0) return 0;
    // Cheap pre-check: below the LOWEST possible floor (the person-shard floor) nothing can rescue, so
    // abstain WITHOUT an SD read — keeps the abstain majority free (the read below is the only SD cost).
    if (bestcos < L1_PERSON_FLOOR) return 0;

    // 3) read the winning bilingual answer record from SD NOW, so the rescue can corroborate the match
    //    lexically (and the person-shard low-floor can verify entity coverage) before accepting.
    if (!l1_read_answer(best_ansoff, en, want_detail, out)) return 0;

    // Absolute floor. EXCEPTION: the person.bin shard, where a bare "chi è <surname>" lands below the
    // normal rescue floor because the card carries only the full name. Lower it to L1_PERSON_FLOOR there,
    // and ONLY when the surname appears EXACTLY in the card (strict coverage) — a typo-neighbour or a
    // fabricated name fails coverage and abstains (gated by the akb5-content person-existence traps).
    bool person_rescue = s_in_akb5 && strstr(s_shard_path, "person.bin")
                         && l1_entity_exact_covered(text, out->reply);
    float abs_floor = person_rescue ? L1_PERSON_FLOOR : rescue_abs;
    if (bestcos < abs_floor) { memset(out, 0, sizeof *out); return 0; }
...
#define AKB5_PROBE     4        // top distinct shards searched per query. Host sweep (tools/anima-host/probe-
                                // sweep.mjs): 3->4 recovers verbose-phrasing misroutes (e.g. "spiegami come
                                // funziona la fotosintesi") at ZERO added hallucinations; 5/6 add cost with no
...
int nucleo_anima_l1_akb5_query(const char *text, bool en, bool want_detail, anima_result_t *out)
{
    ...
    // SCALE-AWARE probe: a person/topic's home shard sits ~rank ns/4 in routing (its cluster is diluted among
    // many shards), so a fixed top-4 misses most once the corpus grows past a few dozen shards. Probe a
    // FRACTION of the shards (clamped) so recall holds at any corpus size — measured: 60 shards needs ~12-15
    // to recover full people recall (12/30 -> 27/30). Cost stays bounded (one shard resident at a time, the
    // rest streamed); ~ns/4 SD shard-loads/query. Tiny corpora (<=6 shards) just probe them all.
    int probe = (int)ns / 4;
