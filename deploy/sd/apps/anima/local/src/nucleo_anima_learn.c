...
// ANIMA Local divergence from firmware: writable state lives under /data/anima/rw/, which the browser
// loader (apps/anima/www/local/engine.js) mounts on Emscripten IDBFS so user-taught facts survive a
// page reload. The read-only pack (encoder/index/cards) stays in MEMFS. Tmp files inherit the rw/ dir
// ("%s.tmp"). On the device/host this subdir is just a normal folder — behaviour is unchanged.
#define U_TSV  NUCLEO_SD_MOUNT "/data/anima/rw/user.tsv"   // id \t trigger \t reply  (one per line)
#define U_VEC  NUCLEO_SD_MOUNT "/data/anima/rw/user.vec"   // u8 idlen | id | u8 dim | int8 vec[dim]
