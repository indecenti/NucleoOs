...
//   {op:'glob',   pattern}
//   {op:'cd',     path}
//   {op:'run',    path}                              a runnable script (.js/.mjs/.cjs)
...
const V = {
  ...
  run:    /\b(esegui|eseguire|lancia|lanciare|run|exec|execute|node)\b/i,
};
...
export function parseFileIntent(text) {
  ...
  // ---- RUN: esegui/run/node <script.js> — only when the object is a runnable script path,
  // else fall through (so "lancia la calcolatrice" stays an app launch, "esegui il backup" a no-op). ----
  if (V.run.test(n)) {
    const path = pickPath(raw);
    if (path && RUNNABLE.test(path)) return { op: 'run', path };
  }
