...
export function loadEngine(onStage) {
  ...
  _loading = (async () => {
    onStage && onStage('Caricamento motore (ffmpeg.wasm, ~30 MB)…');
...
export async function convertFile(file, opts = {}, onStage = () => {}) {
  ...
  let stageLabel = 'Conversione…', stageBase = 0, stageSpan = 100;
  ...
  try {
    onStage('Lettura file…', 0);
    await ff.writeFile(inName, new Uint8Array(await file.arrayBuffer()));

    // 1) audio → mono mp3 (optional; many clips have it, some don't)
    let mp3 = null;
    stageLabel = 'Codifica audio…'; stageBase = 0; stageSpan = 30;
    try {
    ...
    stageLabel = 'Codifica video…'; stageBase = 30; stageSpan = 60;
    ...
    if (!frames.length) throw new Error('nessun frame prodotto (file non supportato?)');

    // 3) assemble .nfv (v2, indexed)
    stageLabel = 'Assemblaggio .nfv…'; stageBase = 90; stageSpan = 9;
...
export async function deliverToDevice(result, onStage = () => {}) {
  const put = async (path, blob) => {
    ...
    if (!r.ok) throw new Error('scrittura fallita (HTTP ' + r.status + ') per ' + path);
  };
  onStage('Scrittura sulla SD del device…', 99);
