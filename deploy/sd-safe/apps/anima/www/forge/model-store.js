...
export const REGISTRY = [
  { id: 'Qwen2.5-Coder-0.5B-Instruct-q4f16_1-MLC', kind: 'webgpu', label: 'Coder 0.5B · GPU (WebLLM)',
    ...
    sdBase: '/apps/anima/www/forge/models/Qwen2.5-Coder-0.5B-Instruct-q4f16_1-MLC/',
    manifest: {
  ...
  { id: 'Qwen2.5-Coder-0.5B-Instruct-GGUF', kind: 'wasm', label: 'Coder 0.5B · CPU (WASM, no GPU)',
    ...
    sdBase: '/apps/anima/www/forge/models/Qwen2.5-Coder-0.5B-Instruct-GGUF/',
    manifest: {
...
export function makeModelStore(deps = {}) {
  ...
  async function fetchOneShard(id, base, file, src, onProgress) {
    if (await deps.cacheHas(id, file.name)) return;                       // idempotent resume: skip cached
    const url = base + file.name;
    let bytes;
    if (src === 'cdn') {
      const r = await deps.fetchWhole(url);                               // CDN: whole-file GET (fast, not the device)
      if (!r || !r.ok) throw new Error('cdn ' + (r && r.status) + ' on ' + file.name);
      bytes = r.bytes;
    } else {
      // SD (Cardputer): SAFE bounded-Range windows, scheduler-gated, never a whole-file GET.
      let win = MAX_WINDOW; const chunks = [];
      ...
      for (let i = 0, rs = ranges(); i < rs.length; i++) {
        ...
        if (d.action === 'pause') throw new Error('paused: ' + d.reason);  // back off — caller retries later
        if (d.action === 'throttle') { win = d.window; rs = ranges(); }    // shrink window, recompute
        const r = await deps.fetchRange(url, rs[i]);
        if (r && r.status === 503) { win = adaptiveWindow(win, true); rs = ranges(); i--; continue; }
        if (!r || !r.ok) throw new Error('sd ' + (r && r.status) + ' on ' + file.name);
        chunks.push(r.bytes);
      }
      bytes = concat(chunks);
    }
    if ((await deps.sha256(bytes)) !== file.sha256) throw new Error('SHA mismatch on ' + file.name);
    await deps.cachePut(id, file.name, bytes);
    onProgress && onProgress({ file: file.name, done: true });
  }

  async function fromSource(id, base, src, onProgress) {
    const { files } = await filesOf(id);
    let done = 0;
    for (const f of files) { await fetchOneShard(id, base, f, src, onProgress); onProgress && onProgress({ phase: 'progress', done: ++done, total: files.length, source: src }); }
  }

  // download(id) — the ONLY entry point, called from a user action. Rejects if another is running.
  // source order: forced `opts.source`, else online→[cdn,sd], offline→[sd]. Returns { ok, source }.
  async function download(id, opts = {}) {
    if (!modelById(id)) throw new Error('unknown model: ' + id);
    if (active) throw new Error('a download is already running (' + active + ') — one at a time');
    active = id;
    try {
      ...
      for (const src of order) {
        ...
        try { await fromSource(id, base, src, opts.onProgress); return { ok: true, source: src }; }
        catch (e) { lastErr = e; opts.onProgress && opts.onProgress({ phase: 'fallback', from: src, error: String(e.message || e) }); }
      }
